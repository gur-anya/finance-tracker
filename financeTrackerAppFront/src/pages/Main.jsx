...
function Main() {
    ...
    const fetchTransactions = async () => {
        ...
        try {
            const response = await apiService.getAllTransactionsWithFilters(filters, currentPage, pageSize);
    ...
    const handlePageSizeChange = (newPageSize) => {
        ...
        setCurrentPage(0); // Сбрасываем на первую страницу при изменении размера страницы
    };
        ...
        <>
                    ...
                    <Pagination>
                        ...
                        {Array.from({ length: Math.min(5, paginationData.totalPages) }, (_, i) => {
                            ...
                            if (paginationData.totalPages <= 5) {
                                ...
                                pageNumber = i;
                            } else if (paginationData.currentPage >= paginationData.totalPages - 3) {
                                pageNumber = paginationData.totalPages - 5 + i;
                            } else {
                            ...
                            return (
                                <Pagination.Item
                                    key={pageNumber}
                                    active={pageNumber === paginationData.currentPage}
            ...
            {paginationData.totalElements > 0 && (
                <div className="text-center mt-2 text-muted">
                    <small>
                        Показано {transactions.length} из {paginationData.totalElements} транзакций 
                        (страница {paginationData.currentPage + 1} из {paginationData.totalPages})
                    </small>
                </div>
            )}
        </>
